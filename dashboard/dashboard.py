# Sentiment Monitoring Dashboard - Streamlit Dual Mode
# Local ML models & DB or remote API mode
...
if "logged_in" not in st.session_state:
    st.session_state.update({
        ...
        "user_id": None,
        "username": None,
        "results_df": pd.DataFrame(columns=["text","sentiment","aspect","confidence","timestamp"]),
        "live_running": False
    })
...
def generate_pdf_report(data):
    ...
    elements += [
        ...
        Paragraph(f"Negative Rate: {negative_rate:.2f}%", styles["Normal"]),
        Paragraph(f"Top Aspect: {top_aspect}", styles["Normal"]),
        Spacer(1,12),
        Paragraph("Aspect Insights:", styles["Heading2"])
    ]
    ...
    with st.sidebar:
        ...
        with tab2:
            ...
            if st.button("Register", key="btn_register"):
                try:
                    safe_pass = new_password.encode("utf-8")[:72].decode("utf-8","ignore")
                    call_register(new_username, safe_pass)
...
if st.session_state["logged_in"]:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Single Message","Batch Upload","Reports & Analytics","Live Feedback"])
    ...
    with tab1:
        ...
        with st.form("single_message_form"):
            ...
            if submitted and user_input.strip():
                try:
                    result = call_predict_sentiment(user_input, st.session_state["user_id"])
                    result["timestamp"] = datetime.now()
                    st.session_state["results_df"] = pd.concat([st.session_state["results_df"], pd.DataFrame([result])], ignore_index=True)
    ...
    def analyze_batch(df_upload):
        ...
        for i, text in enumerate(df_upload["text"]):
            try:
                res = call_predict_sentiment(str(text), st.session_state["user_id"])
                ...
                results.append({"text": text, "sentiment": None, "aspect": None, "confidence": None, "timestamp": datetime.now()})
        ...
        if uploaded_file:
            ...
            if "text" not in df_upload.columns:
                ...
                if st.button("Analyze Uploaded File"):
                    ...
                    st.download_button("Download Batch Results", batch_results.to_csv(index=False).encode("utf-8"), "batch_results.csv", "text/csv")

# You can extend this same structure for Reports & Live Feedback tabs...
